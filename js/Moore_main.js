// Moore Model and Simulator
var model = new MooreModel(); // Change to MooreModel
var Moore = new MooreSimulator(model); // Change to MooreSimulator
...
function createStateElement(id, name) {
    let output = prompt(`Enter output for state ${name}:`, ""); // Prompt for state output
    model.setStateOutput(name, output); // Save state output in the model
    var state = $('<div></div>')
        .attr('data-state-name', name)
        .text(`${name}/${output}`) // Display state name with output
        .attr('id', id)
        .addClass('control')
...
$(".context-menu").on("click", ".delete-state", function (event) {
    ...
    Moore.model.removeTransitions(stateName);
    // Null out start state if it is deleted
    if (Moore.model.startState === stateName) {
        Moore.model.startState = null;
    }
...
$(".context-menu").on("click", ".rename-state", function (event) {
    ...
    Object.keys(Moore.model.transitions).forEach(stateA => {
        Object.keys(Moore.model.transitions[stateA]).forEach(character => {
            Moore.model.transitions[stateA][character].forEach(stateB => {
    ...
    if (nameAlreadyUsed) {
        ...
        Object.keys(Moore.model.transitions).forEach(stateA => {
            Object.keys(Moore.model.transitions[stateA]).forEach(character => {
                Moore.model.transitions[stateA][character].forEach((stateB, index) => {
                    if (stateB === oldName) {
                        Moore.model.transitions[stateA][character][index] = newName;
                    }
                });
            });
        });
        ...
        Object.keys(Moore.model.transitions).forEach(stateA => {
            if (stateA === oldName) {
                Moore.model.transitions[newName] = Moore.model.transitions[oldName];
                Moore.model.transitions[oldName] = {};
            }
        });
        // Update names of starting state if necessary
        if (Moore.model.startState === oldName) {
            Moore.model.startState = newName;
        }
        // Update names of accepting states if necessary
        Moore.model.acceptStates.forEach((acceptState, index) => {
            if (acceptState === oldName) {
                Moore.model.acceptStates[index] = newName;
            }
        });
        // Update DOM element
...
$(".context-menu").on("click", ".make-starting-state", function (event) {
    let oldStart = $("#diagram").find(`[data-state-name='${Moore.model.startState}']`);
    console.log(Moore.model.startState, oldStart)
    let state = window.selectedControl;
    let stateName = state.attr("data-state-name");
    if (!state.hasClass("starting")) {
        state.addClass("starting");
        Moore.model.startState = stateName;
...
$(".context-menu").on("click", ".delete-transition", function (event) {
    ...
    // Remove this transition within the Moore model
    characters.forEach(character => {
        Moore.model.removeTransition(sourceName, character, targetName);
    });
...
$(".context-menu").on("click", ".edit-transition", function (event) {
    ...
    let oldTransitions = [];
    Object.keys(Moore.model.transitions[sourceName]).forEach(character => {
        if (Moore.model.transitions[sourceName][character].indexOf(targetName) > -1) {
    ...
    if (newTransition !== null && newTransition !== "") {
        ...
        newTransitions.forEach(newTrans => {
            if (oldTransitions.indexOf(newTrans) < 0) {
                ...
                Moore.model.addTransition(sourceName, input, targetName, output);
            }
        });
        ...
        oldTransitions.forEach(oldTrans => {
            if (newTransitions.indexOf(oldTrans) < 0) {
                ...
                Moore.model.removeTransition(sourceName, input, targetName);
            }
        })
    }
...
$("#toolbox-wrapper").on("click", "#instant-simulation", function (event) {
    ...
    Moore.initialize(string);
    while (Moore.status === "running") {
        Moore.step();
    }
    outputBox.val(`Output: ${Moore.output}`);
});
...
$("#toolbox-wrapper").on("click", "#start-simulation", function (event) {
    Moore.initialize($('#enter-string').val());
    // Highlight the active state
    let startState = $("#diagram").find(`[data-state-name='${Moore.model.startState}']`);
    ...
    // Initialize tape
    $("#tape-wrapper #start").text(Moore.input.substring(0, Moore.index));
    $("#tape-wrapper #middle").text(Moore.input[Moore.index] || "");
    $("#tape-wrapper #end").text(Moore.input.substring(Moore.index + 1));
    $("#simulation-output").val(`Output: ${Moore.output}`);
});

// Toolbox -> Step simulation
$("#toolbox-wrapper").on("click", "#step-simulation", function (event) {
    highlightedTransitions.forEach(transition => transition.setPaintStyle({ stroke: "black", strokeWidth: 2 }));
    if (Moore.status === "running") {
        ...
        Moore.states.forEach(stateName => {
            // Highlight the active state
...
function onConnection(info) {
    let sourceName = info.source.innerText.split("/")[0]; // Extract state name (ignore output)
    let targetName = info.target.innerText.split("/")[0]; // Extract state name (ignore output)
    let connection = info.connection;

    // Prompt the user for the input characters for this transition
    let transitionCharacters = prompt("Enter the transitions for this connection (input characters separated by commas)."
                                    + "\nFor example: a,b,c will create transitions for inputs a, b, and c.");
    if (transitionCharacters === null || transitionCharacters === "") {
        instance.deleteConnection(connection);
    } else {
        let transitions = transitionCharacters
                            .replace(/[^a-zA-Z0-9_,]/g, "") // Allow only valid characters
                            .toLowerCase()
                            .split(",")
                            .filter(trans => trans !== ""); // Filter out empty entries

        // Add transitions to the Moore model
        transitions.forEach(function (input) {
            model.addTransition(sourceName, input, targetName);
        });
